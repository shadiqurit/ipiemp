    ...
    <form v-else class="form" @submit.prevent="save">
      ...
      <section class="card">
        ...
        <div class="grid">
          ...
          <div class="field"><label>{{ t('Height') }}</label><input v-model="employee.HEIGHT" /></div>
          <div class="field"><label>{{ t('Weight') }}</label><input v-model="employee.WEIGHT" /></div>
          ...
          <div class="field"><label>{{ t('Marital Status') }}</label><select v-model="employee.MARITAL_STATUS"><option value="">Select</option><option value="U">{{ t('Unmarried') }}</option><option value="M">{{ t('Married') }}</option></select></div>
        </div>
      </section>
      ...
      <section class="card">
        ...
        <div class="grid">
          ...
          <div class="field"><label>{{ t('Primary Phone') }}</label><input v-model="employee.PHONE" required /></div>
          <div class="field"><label>{{ t('Alternate Phone') }}</label><input v-model="employee.PHONE1" /></div>
      ...
      <section class="card">
        ...
        <div class="grid"><div class="field"><label>{{ t('Emergency Person') }}</label><input v-model="employee.EMGRCNY_PERSON" /></div><div class="field"><label>{{ t('Relationship') }}</label><input v-model="employee.EMGRCNY_RELATION" /></div><div class="field"><label>{{ t('Emergency Phone') }}</label><input v-model="employee.EMGRCNY_PHONE" /></div><div class="field"><label>{{ t('Emergency Address') }}</label><input v-model="employee.EMGRCNY_ADDRESS" /></div></div>
      </section>

      <section class="card">
        <h2>{{ t('Family & Spouse') }}</h2>
        <div class="grid"><div class="field"><label>{{ t('Father Name') }}</label><input v-model="employee.FATHER_NAME" /></div><div class="field"><label>{{ t('Father Phone') }}</label><input v-model="employee.FATHER_PHONE" /></div><div class="field"><label>{{ t('Mother Name') }}</label><input v-model="employee.MOTHER_NAME" /></div><div class="field"><label>{{ t('Mother Phone') }}</label><input v-model="employee.MOTHER_PHONE" /></div><div class="field"><label>{{ t('Spouse Name') }}</label><input v-model="employee.SPOUSE_NAME" /></div><div class="field"><label>{{ t('Marriage Date') }}</label><DateInput v-model="employee.SPOSE_MARRIAGE_DATE" /></div><div class="field"><label>{{ t('Spouse Occupation') }}</label><input v-model="employee.SPOSE_OCCUPATION" /></div><div class="field"><label>{{ t('Spouse Phone') }}</label><input v-model="employee.SPOUSE_PHONE" /></div></div>
      </section>

      <section class="card">
        <h2>{{ t('Guarantor Information') }}</h2>
        <div class="grid"><div class="field"><label>{{ t('Guarantor Name') }}</label><input v-model="employee.GRNT_NAME" /></div><div class="field"><label>{{ t('Relationship') }}</label><input v-model="employee.GRNT_RELE" /></div><div class="field"><label>{{ t('Guarantor Father') }}</label><input v-model="employee.GRNT_FATHER" /></div><div class="field"><label>{{ t('Present Address') }}</label><input v-model="employee.GRNT_PRESENT_ADD" /></div><div class="field"><label>{{ t('Permanent Address') }}</label><input v-model="employee.GRNT_PERMANET_ADD" /></div><div class="field"><label>{{ t('Nationality') }}</label><input v-model="employee.GRNT_NATIONALITY" /></div><div class="field"><label>{{ t('Profession') }}</label><input v-model="employee.GRNT_PROFFESSION" /></div><div class="field"><label>{{ t('NID') }}</label><input v-model="employee.GRNT_NID" /></div><div class="field"><label>{{ t('Mobile') }}</label><input v-model="employee.GRNT_MOBILE" /></div></div>
      </section>

      <section class="card">
        <div class="section-title"><h2>{{ t('Education') }}</h2><button type="button" @click="education.push(blankEducation())">{{ t('+ Add Education') }}</button></div>
        <div v-for="(item, index) in education" :key="index" class="edu">
          <div class="section-title"><b>{{ t('Education') }} #{{ index + 1 }}</b><button v-if="education.length > 1" type="button" class="danger" @click="education.splice(index, 1)">{{ t('Remove') }}</button></div>
          <div class="grid"><div class="field"><label>{{ t('Exam Name') }}</label><input v-model="item.EXAMNAME" /></div><div class="field"><label>{{ t('Group') }}</label><input v-model="item.EXAMGROUP" /></div><div class="field"><label>{{ t('Board / University') }}</label><input v-model="item.BOARD" /></div><div class="field"><label>{{ t('Class / Result') }}</label><input v-model="item.CLAS" /></div><div class="field"><label>{{ t('Pass Year') }}</label><input v-model="item.PASSYEAR" /></div><div class="field"><label>{{ t('Subject') }}</label><input v-model="item.SUBJECT_NAME" /></div><div class="field"><label>{{ t('Institute') }}</label><input v-model="item.INSTITUTE" /></div><div class="field"><label>{{ t('Remarks') }}</label><input v-model="item.REMARKS" /></div></div>
        </div>
      </section>
